...
function OfferCard({ offer }: { offer: Offer }) {
  return (
    ...
    >
      <Link
        href={offer.href}
        className="group relative flex h-[380px] w-[286px] shrink-0 overflow-hidden rounded-[28px] border border-olive-primary/10 bg-[#f4efe6] shadow-[0_24px_60px_rgba(79,93,60,0.10)] transition-transform duration-300 hover:-translate-y-1 sm:h-[408px] sm:w-[312px]"
      >
        <div className="absolute inset-x-0 top-0 h-[52%] overflow-hidden">
          <Image
            ...
            alt={offer.imageAlt}
            fill
            sizes="(max-width: 640px) 286px, 312px"
            className="object-cover transition-transform duration-700 group-hover:scale-105"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#f4efe6] via-[#f4efe6]/10 to-black/10" />
        </div>

        <div className="absolute bottom-0 left-0 right-0 flex h-[53%] flex-col justify-between rounded-t-[30px] bg-[linear-gradient(180deg,rgba(244,239,230,0.92)_0%,#f4efe6_18%,#ffffff_100%)] p-5">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 rounded-full bg-olive-primary/8 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-olive-primary/70">
              <Tag className="h-3.5 w-3.5" />
              <span>{offer.tag}</span>
            </div>
            <div className="space-y-2">
              <h3 className="text-[1.45rem] font-semibold leading-tight text-olive-primary">
                {offer.title}
              </h3>
              <p className="text-sm leading-6 text-olive-primary/72">
                {offer.description}
              </p>
            </div>
          </div>

          <div className="flex items-end justify-between gap-4 border-t border-olive-primary/10 pt-4">
            <div className="flex min-w-0 items-center gap-3">
              <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border border-olive-primary/10 bg-white">
                <Image
              ...
              <div className="min-w-0">
                <p className="truncate text-xs font-semibold uppercase tracking-[0.14em] text-olive-primary/55">
                  {offer.brandName}
                </p>
                <p className="truncate text-base font-semibold text-olive-primary">
                  {offer.promoCode ?? offer.price ?? 'Подробнее'}
                </p>
              </div>
            </div>

            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-olive-primary text-white transition-all duration-300 group-hover:-rotate-45 group-hover:bg-olive-light">
  ...
  ({ className, defaultCategory = 'popular', ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'relative overflow-hidden rounded-[36px] border border-olive-primary/10 bg-[linear-gradient(180deg,#ffffff_0%,#f6f1e8_100%)] px-4 py-6 shadow-[0_30px_80px_rgba(79,93,60,0.10)] sm:px-6 sm:py-7 lg:px-8',
          className,
        )}
        {...props}
      >
        ...
        <div className="relative z-[1] mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-olive-primary/10 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-olive-primary/60">
              <Sparkles className="h-3.5 w-3.5" />
              <span>IV Therapy BIORISE</span>
            </div>
            <p className="text-sm leading-6 text-olive-primary/72 sm:text-[15px]">
              Подбор по цели, анализам и текущему состоянию. Каждая программа оформлена как отдельный сценарий восстановления.
            </p>
          </div>
        ...
        <div className="relative z-[1] mb-6 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <div
          ...
          className="relative z-[1] flex snap-x snap-mandatory gap-4 overflow-x-auto pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-5"
        >
